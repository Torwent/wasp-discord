import type { Button } from "$lib/interaction"
...
const modal: Button = {
	name: "subscriptions_previous",
	description: "Subscriptions previous button",
	...
	run: async ({ member, interaction }) => {
		...
		const msg = await getSubscriptions(member, wsid, page - 2)
