...
row.addComponents(
	new ButtonBuilder().setCustomId("solve").setLabel("Solved").setStyle(ButtonStyle.Success)
)
...
row.addComponents(
	new ButtonBuilder().setCustomId("lock").setLabel("Close & Lock").setStyle(ButtonStyle.Danger)
)
...
export async function threadReply(interaction: ModalExtendedInteraction | MenuExtendedInteraction) {
