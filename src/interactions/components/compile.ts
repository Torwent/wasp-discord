...
const scriptReply = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
	new StringSelectMenuBuilder()
...
export async function compileReply(interaction: MenuExtendedInteraction) {
	...
	await interaction.editReply(replyMsg(rowCompile)).catch((error) => console.log(error))
}
...
export async function modalCompileReply(interaction: ModalExtendedInteraction) {
	...
	await interaction.editReply(replyMsg(scriptReply)).catch((error) => console.log(error))
}
